...
redisClient.connect(); // Required for Redis v4+

redisClient.on("error", (err) => {
	console.error("Redis error:", err);
});

// Custom base-64 character set matching your Godot encoding
const BASE64_CHARS = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

// Helper: Encode integer to custom base-64
function intToBase64(intValue) {
	let result = "";
	do {
		result = BASE64_CHARS[intValue % 64] + result;
		intValue = Math.floor(intValue / 64);
	} while (intValue > 0);
	return result;
}
...
app.post("/submit-placement", async (req, res) => {
	...
	if (Object.keys(placements).length === 2) {
		...
		const current_turn = 0; // You could randomize this
		const game_state_code = 4; // 'playing' state

		const game_state_bits =
			(1 << 6) | ((current_turn & 0b1) << 0) | ((game_state_code & 0b1111) << 1);
		const game_base64 = intToBase64(game_state_bits);
